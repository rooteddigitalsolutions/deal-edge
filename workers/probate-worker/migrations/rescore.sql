...
SELECT
  ...
  CASE
    WHEN UPPER(COALESCE(p.land_use,'')) LIKE '%UNUSED%'
      ...
      OR p.land_use LIKE '100%' OR p.land_use LIKE '19%' OR p.land_use LIKE '39%'
      THEN 'vacant'
    WHEN p.land_use LIKE '101%' OR p.land_use LIKE '102%' OR p.land_use LIKE '124%'
      OR p.land_use LIKE '200%'   -- TAX FREEZE: still a residence
      OR UPPER(COALESCE(p.land_use,'')) LIKE '%FAMILY%'
      OR UPPER(COALESCE(p.land_use,'')) LIKE '%CONDO%'
      OR UPPER(COALESCE(p.land_use,'')) LIKE '%RES%'
      THEN 'house'
    WHEN p.land_use IS NULL OR p.land_use=''
      THEN 'unknown'   -- land_use not yet loaded; distinguish from true 'other'
    ELSE 'other'
